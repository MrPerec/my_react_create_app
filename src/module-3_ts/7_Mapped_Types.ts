 ...
 * Эта запись присвоит переменной TMyBigObject структуру типов объекта bigObject и его содержимого:
 * 
 * type TMyBigObject = {
    rc: number;
    rcTrue: boolean;
...
/** Напишем свой собственный тип Partial
 * который делает все клучи в объекте не обязательными */
type MyPartial<T> = {
  [N in keyof T]?: T[N];
};
