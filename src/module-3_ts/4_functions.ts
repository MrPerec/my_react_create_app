...
возвращать undefined но она его не вернёт т.к. мы указали возвращаемый тип number и TS будет ругаться, 
можно убрать возвращаемый тип number и тогда undefined будет возвращаться 
 ...
 * а alias предпочтительней c ф-ми
*/
...
/** разберёмся со странными типами которые вводит TS */
...
/* тип any нет в JS и когда мы используем его в для указания типа мы как бы отключаем типизаци
   any лучше не использовать за исключением редких случаев, 
   например когда есть  какая-то ф-я которая возвращает не понятный резултат и следовательно тип
*/
const some: any = '';
